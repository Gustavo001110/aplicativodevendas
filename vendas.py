...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            local_id, id_token = self.fire.trocar_token(refresh_token)
            self.local_id = local_id
            self.id_token = id_token

            # pegar informações do usuario
            requisicao = requests.get(f"https://aplicativovendas2-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
            ...
            pagina_ajustes.ids["id_vendedor"].text = f"Seu ID: {id_vendedor}"
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativovendas2-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                    data=info)
        self.avatar = foto
        self.mudar_tela("ajustespage")

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendas2-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}" '
        ...
        if requisicao_dic == {}:
            mensagem_texto.text = 'usuario nao encontrado'
        else:
            equipe = self.equipe.split(",")
            if id_vendedor_adicionado in equipe:
                mensagem_texto.text = 'vendedor já esta adicionado na sua lista'
            else:
                self.equipe = self.equipe + f',{id_vendedor_adicionado}'
                info = f'{{"equipe": "{self.equipe}"}}'
                requests.patch(f"https://aplicativovendas2-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}", data=info)
                mensagem_texto.text = 'vendedor adicionado com sucesso'
                # adicionar um novo vendedor na lista
                pagina_listavendedores = self.root.ids["listarvendedorespage"]
                lista_vendedores = pagina_listavendedores.ids["lista_vendedores"]
    ...
    def adicionar_venda(self):
        ...
        if not quantidade:
            ...
            try:
                ...
                pagina_adicionarvendas.ids['label_quantidade'].color = (1, 0, 0, 1)
        # adicionar a venda no bando de dados
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (
            ...
            requests.post(f"https://aplicativovendas2-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                          data=info)
            banner = BannerVendas(cliente=cliente, produto=produto, foto_cliente=foto_cliente,
                                  foto_produto=foto_produto, data=data, preco=preco, quantidade=quantidade,
                                  unidade=unidade)
            pagina_homepage = self.root.ids["homepage"]
            lista_vendas = pagina_homepage.ids["lista_vendas"]
            lista_vendas.add_widget(banner)
            ...
            requisicao = requests.get(
                f'https://aplicativovendas2-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}')
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(f'https://aplicativovendas2-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
        ...
        for local_id_usuario in requisicao_dic:
            try:
                ...
                for id_venda in vendas:
                    ...
                    banner = BannerVendas(cliente=venda['cliente'], produto=venda['produto'],
        ...
        # preencher o total de vendas
        pagina_todasvendas.ids["label_total_vendas"].text = f"Total de Vendas: R${total_vendas}"

        # direcionar pra pagina de vendas
        self.mudar_tela('todasvendaspage')

    def sair_todas_vendas(self, id_tela):
        # mudar a foto para a de perfil dnv
        foto_perfil = self.root.ids['foto_perfil']
    ...
    def carregar_vendas_vendedor(self, dic_info_vendedor, *args):
        try:
            ...
            for id_venda in vendas:
                venda = vendas[id_venda]
                banner = BannerVendas(cliente=venda["cliente"], produto=venda["produto"], foto_cliente=venda["foto_cliente"],
                                     foto_produto=venda["foto_produto"], data=venda["data"],
